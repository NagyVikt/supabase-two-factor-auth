...
// CORRECTED: Import the server action for recovery
import { enrollMFA } from '@/lib/actions/mfa/enrollMfa';
import { verifyMFA } from '@/lib/actions/mfa/verifyMfa';
import { recoverMfa } from '@/lib/actions/mfa/recoverMfa'; // This is the action to call
...
export default function MfaVerification() {
  ...
  const handleRecover = async () => {
    ...
    try {
      // No more fetch! We call the server action directly.
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-neutral-900 p-4 font-sans">
      <div className={`w-full max-w-xl bg-white dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-lg shadow-md p-6 sm:p-8 flex flex-col md:flex-row gap-6 sm:gap-8 transition-all duration-300`}>
        ...
        <div className={`flex-1 flex flex-col justify-center ${!qrCode ? 'w-full md:max-w-sm mx-auto' : ''}`}>
          ...
          {error && <div className="mb-4 px-3 py-2 bg-red-50 dark:bg-red-900/30 border border-red-500 text-red-700 dark:text-red-300 rounded-md text-sm flex items-center gap-2"><Icon icon="mdi:alert-circle-outline" className="text-lg"/><span>{error}</span></div>}
          {recoverMsg && <div className="mb-4 px-3 py-2 bg-green-50 dark:bg-green-900/30 border border-green-500 text-green-700 dark:text-green-300 rounded-md text-sm flex items-center gap-2"><Icon icon="mdi:check-circle-outline" className="text-lg"/><span>{recoverMsg}</span></div>}
